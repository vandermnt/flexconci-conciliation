...
class VendasController extends Controller{
  ...
  public function buscarVendasFiltro(){
    ...
    try {
      ...
      ->where(function($query) {
        $filtro_tabela = Request::only('filtro_tabela');
        $valor_digitado = Request::only('valor_digitado');
        if($filtro_tabela['filtro_tabela'] != null){
          $query->where($filtro_tabela['filtro_tabela'] , 'like', '%' . $valor_digitado['valor_digitado'] . '%');
        }
      })
      ->where(function($query) {
      ...
      ->where(function($query) {
        ...
        $val_tarifa_minima = DB::table('vendas')
        ->selectRaw('sum(TAXA_MINIMA) as tx_minima')
        ->where('vendas.COD_CLIENTE', '=', session('codigologin'))
        ->where(function($query) {
          if(Request::only('array') != null){
            $empresas = Request::only('array');
          ...
          ->where(function($query) {
            ...
            ->where(function($query) {
              ...
              ->where(function($query) {
                ...
                $val_tarifa_minima = number_format($val_tarifa_minima->tx_minima, 2,",",".");
                $val_taxa_percent = number_format($val_taxa_percent->val_taxa_percent, 2, '.', '');
                $val_taxas = number_format($val_taxas, 2,",",".");

                $vendas = json_encode([$vendas, $val_liquido, $val_bruto, $qtde_registros, $val_taxas, $val_taxa_soma, $val_taxa_percent, $val_tarifa_minima]);
