...
<body>
  <div class="row">
    <div class="col-sm-2">
      <img src="../public/assets/images/logoconci.png" style="width: 150px;" alt="Logo Conciflex">
    </div>
    <div class="col-sm-10">
      <h5 style="text-align: center"> Vendas por Operadora </h5><br>
    </div>
  </div>
  <div style="margin-left: -35px !important">
    <table id="table_vendas_produto" class="table" style="font-size: 12px">
      ...
      <tbody>
        ...
        <tr>
          <td> {{ $dado_venda->ADQUIRENTE }}</td>
          <td> {{ $dado_venda->QUANTIDADE_REAL }}</td>
          <td> <?php $bruto =  number_format($dado_venda->TOTAL_BRUTO, 2, ',', '.'); ?> R$ {{ $bruto }}</td>
          <td style="color: red"> <?php $taxa =  number_format($dado_venda->TOTAL_TAXA, 2, ',', '.'); ?> R$ {{ $taxa }}
          </td>
